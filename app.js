...
/* ---------- premium 3D hero icon (larger, glossy, lit) ---------- */
function heroIcon(key, night = false) {
  const defs = `<defs>
  ...
  const sun = (cx, cy, r) => {
    let rays = '';
    for (let i = 0; i < 8; i++) { const a = (i * Math.PI) / 4; rays += `<line class="wc-ray" x1="${cx + Math.cos(a) * (r + 5)}" y1="${cy + Math.sin(a) * (r + 5)}" x2="${cx + Math.cos(a) * (r + 14)}" y2="${cy + Math.sin(a) * (r + 14)}"/>`; }
  ...
  return `<svg viewBox="0 0 100 100" class="wc wc--${key} wc--hero">${defs}${ground}${body}</svg>`;
}
...
function render() {
  ...
  // current
  $('#currentIcon').innerHTML = heroIcon(info.icon, night);
